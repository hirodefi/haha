...
const availableConnectors = [
  ...
  },
  {
    ...
    mobile: true,
    mobileOnly: true,
    href: "https://metamask.app.link/dapp/fantoon.com/",
  },
  {
    type: "walletconnect",
    name: "WalletConnect",
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      var id = window.localStorage.getItem("walletconnect");
      try {
          if (id && id.length > 0) {
              this.connectToWallet(id);
          }
  ...
  methods: {
    async connectToWallet(id) {
      try {
        ...
        window.localStorage.setItem("walletconnect", id);
